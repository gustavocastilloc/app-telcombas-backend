...
import ec.pacifico.apinvent.facade.ModeloFacade;
...
public class ModeloWS {
    ...
    public Response list(@Context HttpServletRequest request, Modelo dato) {
        ...
        if (dato.getIdMarca()!=null && dato.getIdMarca()==0) {
            dato.setIdMarca(null);
        } 
        if (dato.getIdEquipo()!=null && dato.getIdEquipo()==0) {
            dato.setIdEquipo(null);
        } 
        ...
        try {
            con = new Conexion();
            ModeloFacade consultaFacade = new ModeloFacade(con);
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porId(@Context HttpServletRequest request, Modelo dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNombre(@Context HttpServletRequest request, Modelo dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response crear(@Context HttpServletRequest request, Modelo dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if (dato.getIdEquipo()== null) {
            resp.put("log", "Ingresar correctamente: equipo");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getIdMarca()== null) {
            resp.put("log", "Ingresar correctamente: marca");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        ...
        if (dato.getIdFlash()== 0) {
            dato.setIdFlash(null);
        }
        dato.setNombre(dato.getNombre().trim().toUpperCase());

        if(dato.getFecha().toLowerCase().equals("date") || dato.getFecha().toLowerCase().contains("fecha")){
            dato.setFecha(null);
        }
        ...
        try {
            ...
            if (consultaFacade.existeModelo(dato.getNombre())) {
                System.out.println("Modelo con el nombre '" + dato.getNombre() + "' ya existe.");
                resp.put("log", "Ya existe MODELO!!!, considere poner uno nuevo");
                return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                        build();
            }
...
*           */
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response actualizar(@Context HttpServletRequest request, Modelo dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if (dato.getIdEquipo()== null) {
            resp.put("log", "Ingresar correctamente: id equipo");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getIdFlash()== null) {
            resp.put("log", "Ingresar correctamente: id flash");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getIdMarca()== null) {
            resp.put("log", "Ingresar correctamente: id marca");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getIdRam()== null) {
            resp.put("log", "Ingresar correctamente: id ram");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getNombre() == null || dato.getNombre().trim().length() == 0) {
