...
import ec.pacifico.apinvent.facade.ConfigActividadFacade;
...
@LocalBean
@Path("configact")
public class ConfigActividadWS {

    @POST
    @Path("list")
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response list(@Context HttpServletRequest request, ConfigActividad dato) {
        ...
        try {
            con = new Conexion();
            ConfigActividadFacade consultaFacade = new ConfigActividadFacade(con);
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porId(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response opcion(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if (dato.getSubactop() == null) {
            resp.put("log", "Ingresar correctamente: Filtro Subactividades");
            resp.put("info", new JSONArray());
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        ...
        try {
            con = new Conexion();
            ConfigActividadFacade consultaFacade = new ConfigActividadFacade(con);
            op = consultaFacade.obtenerOpciones(dato);
            con.closeConnection();
        } catch (Exception e) {
            System.out.println(this.getClass().toString() + " " + e.getMessage());
        } finally {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNomAct(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        try {
            con = new Conexion();
            ConfigActividadFacade consultaFacade = new ConfigActividadFacade(con);
            lista = consultaFacade.busquedaNombre(dato, 0);
            con.closeConnection();
        } catch (Exception e) {
            System.out.println(this.getClass().toString() + " " + e.getMessage());
        } finally {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNomSubact(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        try {
            con = new Conexion();
            ConfigActividadFacade consultaFacade = new ConfigActividadFacade(con);
            lista = consultaFacade.busquedaNombre(dato, 1);
            con.closeConnection();
        } catch (Exception e) {
            System.out.println(this.getClass().toString() + " " + e.getMessage());
        } finally {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response crear(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if(dato.getIssub()==1 &&
          ((dato.getSubact0()!=null && dato.getSubact0().trim().length()>0) ||
           (dato.getSubact1()!=null && dato.getSubact1().trim().length()>0) ||
           (dato.getSubact2()!=null && dato.getSubact2().trim().length()>0) ||
           (dato.getSubact3()!=null && dato.getSubact3().trim().length()>0) )){
            resp.put("log", "Subactividad no ingresa opciones NA - I/R de equipo - Wireless");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }

        if (dato.getSubact0() != null && dato.getSubact0().trim().length() == 0) {
            dato.setSubact0(null);
        }
        if (dato.getSubact1() != null && dato.getSubact1().trim().length() == 0) {
            dato.setSubact1(null);
        }
        if (dato.getSubact2() != null && dato.getSubact2().trim().length()== 0) {
            dato.setSubact2(null);
        }
        if (dato.getSubact3() != null && dato.getSubact3().trim().length() == 0) {
            dato.setSubact3(null);
        }

        if (dato.getSubact0() != null && !dato.getSubact0().matches("^[0-9]{0,12}([,][0-9]{0,12})*?$")) {
            resp.put("log", "Ingresar correctamente: N/A");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getSubact1() != null && !dato.getSubact1().matches("^[0-9]{0,12}([,][0-9]{0,12})*?$")) {
            resp.put("log", "Ingresar correctamente: Ingreso/Retiro de Equipo");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getSubact2() != null && !dato.getSubact2().matches("^[0-9]{0,12}([,][0-9]{0,12})*?$")) {
            resp.put("log", "Ingresar correctamente: Wireless");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        if (dato.getSubact3() != null && !dato.getSubact3().matches("^[0-9]{0,12}([,][0-9]{0,12})*?$")) {
            resp.put("log", "Ingresar correctamente: Ingreso/Retiro de Equipo - Wireless");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        ...
        try {
            ...
            ConfigActividadFacade consultaFacade = new ConfigActividadFacade(con);
            if (consultaFacade.busquedaNombre(dato, dato.getIssub()) != null) {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response actualizar(@Context HttpServletRequest request, ConfigActividad dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
