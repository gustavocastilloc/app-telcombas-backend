...
import ec.pacifico.apinvent.facade.AgenciaFacade;
...
public class AgenciaWS {
    ...
    public Response list(@Context HttpServletRequest request, Agencia dato) {
        ...
        try {
            con = new Conexion();
            AgenciaFacade consultaFacade = new AgenciaFacade(con);
            lista = consultaFacade.obtenerListadoall(dato);
            size = consultaFacade.obtenerListadoAllSize(dato);
            con.closeConnection();
        } catch (Exception e) {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response tickets(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if (dato.getIdproveedor() != null && dato.getIdproveedor() == 0) {
            dato.setIdproveedor(null);
        }

        if (dato.getCiudad() != null && dato.getCiudad().length() == 0) {
            dato.setCiudad(null);
        }

        if (dato.getTipo() != null && dato.getTipo().length() == 0) {
            dato.setTipo(null);
        }
        ...
        try {
            con = new Conexion();
            AgenciaFacade consultaFacade = new AgenciaFacade(con);
            lista = consultaFacade.obtenerListadoTickets(dato);
            con.closeConnection();
        } catch (Exception e) {
            System.out.println(this.getClass().toString() + " " + e.getMessage());
        } finally {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porId(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNombre(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNombre2(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        try {
            con = new Conexion();
            AgenciaFacade consultaFacade = new AgenciaFacade(con);
            lista = consultaFacade.busquedaNombre2(dato);
            con.closeConnection();
        } catch (Exception e) {
            System.out.println(this.getClass().toString() + " " + e.getMessage());
        } finally {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response crear(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response actualizar(@Context HttpServletRequest request, Agencia dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        try {
            ...
            if (dato.getEstado() == 0) {
                if (consultaFacade.existenEnlaces(dato)) {
                    resp.put("log", "Existen enlaces para esta agencia");
                    return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                            build();
                }
                if (consultaFacade.existenEquipos(dato)) {
                    resp.put("log", "Existen equipos para esta agencia");
                    return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                            build();
                }
            }
