...
import ec.pacifico.apinvent.facade.PerfilFacade;
...
public class PerfilWS {
    ...
    public Response list(@Context HttpServletRequest request, Perfil dato) {
        ...
        try {
            con = new Conexion();
            PerfilFacade consultaFacade = new PerfilFacade(con);
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porId(@Context HttpServletRequest request, Perfil dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response porNombre(@Context HttpServletRequest request, Perfil dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response crear(@Context HttpServletRequest request, Perfil dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        if (dato.getInformacion()== null) {
            resp.put("log", "Ingresar correctamente: informacion");
            resp.put("info", new JSONArray());
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }

        if (dato.getEnlaces()== null) {
            resp.put("log", "Ingresar correctamente: enlaces");
            resp.put("info", new JSONArray());
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }

        if (dato.getEliminar() == null) {
            resp.put("log", "Ingresar correctamente: eliminar");
            resp.put("info", new JSONArray());
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }

        if (dato.getAdministrar() == null) {
            resp.put("log", "Ingresar correctamente: administrar");
            resp.put("info", new JSONArray());
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }
        /*
        if (dato.getPassword()== null || dato.getPassword().trim().length() == 0) {
            resp.put("log", "Ingresar correctamente: *contraseña");
            return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                    build();
        }*/
        ...
        try {
            ...
            if (lista2 != null) {
                ...
                if (!(boolean) obj.get("administrar")) {
                    resp.put("log", "Usuario no tiene permisos de administracion, revisar con usuario de Inventario Outsourcing ");
                    return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                            build();
                }
            ...
            PerfilFacade consultaFacade = new PerfilFacade(con);
            if (consultaFacade.busquedaNombre2(dato) != null) {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public Response actualizar(@Context HttpServletRequest request, Perfil dato) {
        JSONObject resp = new JSONObject();
        if (request.getContentType().isEmpty() || request.getContentType() == null || request.getContentType().isBlank()
                || !request.getContentType().contains("application/json") || request.getContentLength() == 0) {
            resp.put("log", "No ha enviado datos");
        ...
        try {
            ...
            if (lista2 != null) {
                ...
                JSONObject obj = (JSONObject) parser.parse(lista2.get(0).toString());
                if (!(boolean) obj.get("administrar")) {
                    resp.put("log", "Usuario no tiene permisos de administracion, revisar con usuario de Admin Outsourcing ");
                    return Response.status(Response.Status.BAD_REQUEST).entity(resp.toString()).
                            build();
                }
